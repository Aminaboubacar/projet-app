...
import com.cnss.ne.service.PosteQueryService;
...
/**
 * REST controller for managing {@link com.cnss.ne.domain.Poste}.
 */
@RestController
...
public class PosteResource {
    ...
    public ResponseEntity<Poste> updatePoste(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Poste poste)
        ...
        Poste result = posteService.update(poste);
        return ResponseEntity
            .ok()
    ...
    public ResponseEntity<Poste> partialUpdatePoste(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, poste.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<Poste>> getAllPostes(
        ...
        Page<Poste> page = posteQueryService.findByCriteria(criteria, pageable);
