...
type RestOf<T extends IAgent | NewAgent> = Omit<T, 'dateNaissance' | 'dateDece'> & {
...
export class AgentService {
  ...
  protected convertDateFromClient<T extends IAgent | NewAgent | PartialUpdateAgent>(agent: T): RestOf<T> {
    return {
      ...agent,
      dateNaissance: agent.dateNaissance?.toJSON() ?? null,
      dateDece: agent.dateDece?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAgent: RestAgent): IAgent {
    return {
      ...
      dateDece: restAgent.dateDece ? dayjs(restAgent.dateDece) : undefined,
    };
  }
